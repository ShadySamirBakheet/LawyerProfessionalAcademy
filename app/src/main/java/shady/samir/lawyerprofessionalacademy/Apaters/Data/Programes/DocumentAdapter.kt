...
import shady.samir.lawyerprofessionalacademy.View.Programme.PdfViewerActivity

class DocumentAdapter (private val context: Context?) : RecyclerView.Adapter<DocumentAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.apply {
            itemView.setOnClickListener {
                if (context != null) {
                    context.startActivity(Intent(context, PdfViewerActivity::class.java).putExtra("idNotify",1))
                }
            }
        }
        holder.apply {
